...
public class ProviderSearchService {
    ...
    public Object searchKeyword(String keyword, FieldParam field, OrderType order) {
        if (!redisTemplate.hasKey(keyword)) {
            return searchResult(keyword, providerEsService.searchKeyword(keyword, field, order));
        } else {
